...
describe("ProgressView", () => {
  ...
  it("shows the untouched state when nothing is stored", () => {
    ...
    for (const { card, history } of items) {
      ...
      expect(
        within(card).getByText("Start your first round")
      ).toBeInTheDocument();
  ...
  it("reflects stored progress in topic order", () => {
    ...
    expect(
      within(css.card).getByText("Last played 2 days ago")
    ).toBeInTheDocument();
    expect(css.history).toEqual(["4", "10/10", "2 days ago"]);
    expect(
      within(html.card).getByText("Not attempted yet")
    ).toBeInTheDocument();
    expect(
      within(js.card).getByText("4 questions to review")
    ).toBeInTheDocument();
    expect(
      within(js.card).getByText("8/10", { exact: false })
    ).toBeInTheDocument();

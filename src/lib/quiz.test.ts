...
describe("select", () => {
  ...
  it("returns the same state when re-selecting the current option", () => {
    ...
    expect(quizReducer(selected, { type: "select", optionId: "c" })).toBe(selected)
  })
})
...
describe("getQuizResult", () => {
  it.each([
    ["no answers", {}, 0, 0],
    ["all correct", Object.fromEntries(questions.map((q) => [q.id, q.correctOptionId])), 10, 100],
    ["all wrong", Object.fromEntries(questions.map((q) => [q.id, wrong(q.correctOptionId)])), 0, 0],
  ...
  it("treats questions missing from the answers as skipped", () => {
    ...
    expect(result.answers.every((row) => row.selectedOptionId === null)).toBe(true)
  })
})

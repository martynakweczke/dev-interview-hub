...
async function play(picks: (OptionId | null)[], durationMs = 60_000) {
  ...
  for (const [index, pick] of picks.entries()) {
    ...
    if (pick === null) {
      ...
      await user.click(screen.getAllByRole("radio")[OPTION_ORDER.indexOf(pick)])
...
function rows() {
  const table = screen.getByRole("table", { name: "JavaScript question breakdown" })
...
describe("ResultsView", () => {
  ...
  it("renders a 7/10 round with a new best, the delta and the breakdown", async () => {
    ...
    const picks = questions.map((question, index): OptionId | null => {
      if (index === 3) return wrong(question.correctOptionId)
      if (index === 7) return null
      if (index === 9) return wrong(question.correctOptionId)
    ...
    expect(screen.getByText("3 to review. Retry to lock them in.")).toBeInTheDocument()
    ...
    expect(within(time).getByText("4:12")).toHaveAttribute("aria-hidden", "true")
    expect(within(time).getByText("4 minutes 12 seconds")).toHaveClass("sr-only")

    const breakdown = rows()
    expect(breakdown).toHaveLength(10)
    expect(
      breakdown.map((row) => row.getAttribute("data-result"))
    ).toEqual([
      "correct", "correct", "correct", "incorrect", "correct",
      "correct", "correct", "skipped", "correct", "incorrect",
    ])
    expect(within(breakdown[7]).getAllByRole("cell")[1]).toHaveTextContent("Skipped")
  ...
  it("renders a perfect first round without a delta badge", async () => {
    ...
    expect(screen.getByRole("img", { name: "Score: 10 out of 10, 100%" })).toBeInTheDocument()
    expect(screen.getByText("New personal best")).toBeInTheDocument()
    expect(screen.queryByText(/vs last round/)).not.toBeInTheDocument()
    expect(screen.getByText("A perfect score. Retry any time to keep it sharp.")).toBeInTheDocument()
    expect(rows().every((row) => row.getAttribute("data-result") === "correct")).toBe(true)
    expect(storedJs()).toMatchObject({ bestScore: 10, attempts: 1, lastScore: 10 })
  })
  ...
  it("renders a skipped 0/10 round with no badges", async () => {
    ...
    expect(screen.getByRole("img", { name: "Score: 0 out of 10, 0%" })).toBeInTheDocument()
    expect(screen.queryByText("New personal best")).not.toBeInTheDocument()
    expect(screen.queryByText(/vs last round/)).not.toBeInTheDocument()
    expect(
      screen.getByRole("heading", { level: 1, name: "Good start — let's review the fundamentals." })
    ).toBeInTheDocument()
    for (const row of rows()) {
      expect(row).toHaveAttribute("data-result", "skipped")
      expect(within(row).getAllByRole("cell")[1]).toHaveTextContent("Skipped")
    }
    expect(storedJs()).toMatchObject({ bestScore: 8, attempts: 2, lastScore: 0 })
  })
})
